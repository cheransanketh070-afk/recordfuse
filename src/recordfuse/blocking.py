...
class BlockingIndex:
    ...
    Oversized buckets are skipped to prevent generic names or shared email domains from
    turning into accidental quadratic work. Exact email remains its own highly selective key.
    """
    ...
    def keys(self, record: EntityRecord) -> set[str]:
        keys: set[str] = set()
        email = self.registry.normalize("email", record.value("email"))
        phone = self.registry.normalize("phone", record.value("phone"))
        name = self.registry.normalize("name", record.value("name"))
        ...
        if email:
            ...
            if "@" in email:
                ...
                if len(local) >= 3:
                    keys.add(f"email-local-domain:{local[:3]}:{domain}")
        if phone:
            keys.add(f"phone-suffix:{phone[-self.phone_suffix_length:]}")
        if name:
            tokens = name.split()
            if tokens:
            ...
            if len(tokens) >= 2:
                keys.add(f"name-initials:{tokens[0][0]}:{tokens[-1]}")
        return keys

    def candidate_pairs(self, records: list[EntityRecord]) -> list[tuple[EntityRecord, EntityRecord]]:
        ...
        for key in sorted(buckets):
            bucket = sorted(buckets[key], key=lambda record: record.record_id)
        ...
        result.sort(key=lambda pair: (pair[0].record_id, pair[1].record_id))
        self.last_stats = BlockingStats(len(records), len(buckets), skipped, len(result))
        return result
