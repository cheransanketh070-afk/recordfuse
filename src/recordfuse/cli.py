...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        prog="recordfuse",
        description="Explainable entity reconciliation",
    )
    ...
    reconcile_parser = subparsers.add_parser(
        "reconcile",
        help="Reconcile CSV/JSON/JSONL datasets",
    )
...
def run_cli(
    ...
    if not input_files:
        raise SystemExit(
            "reconcile requires at least one input file"
        )
    ...
    print(
        ...
        f"{result.metrics['clusters']} clusters, "
